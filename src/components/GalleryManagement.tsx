...
export default function GalleryManagement() {
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-hidden">
      <div className="flex gap-2 sm:gap-3 p-1 sm:p-1.5 bg-white/5 rounded-3xl w-fit border border-white/30 backdrop-blur-xl ml-0">
        <button 
          className={`flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 rounded-2xl font-black uppercase tracking-widest text-[9px] sm:text-[10px] transition-all ${activeTab === 'images' ? 'bg-black/10 text-white shadow-xl' : 'text-white/40 hover:text-white'}`}
          onClick={() => setActiveTab('images')}
        >
          <ImageIcon className="w-3 sm:w-3.5 h-3 sm:h-3.5" /> Photos
        </button>
        <button 
          className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all ${activeTab === 'videos' ? 'bg-black/10 text-white shadow-xl' : 'text-white/40 hover:text-white'}`}
      ...
      <div key={activeTab} className="p-4 sm:p-8 rounded-3xl bg-transparent border border-white/30 shadow-2xl backdrop-blur-xl bento-card relative overflow-hidden w-full">
        ...
        {activeTab === 'images' ? (
          <div className="space-y-6">
            <div>
              ...
              <select value={imgCategory} onChange={(e) => setImgCategory(e.target.value as Category)} className="w-full h-14 rounded-2xl border border-white/30 bg-white/5 px-4 focus:ring-2 focus:ring-blue-500/20 text-white font-bold">
            ...
            {imgCategory && (
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-1">
                  ...
                  <select value={selectedAlbumId} onChange={(e) => setSelectedAlbumId(e.target.value)} className="w-full h-14 rounded-2xl border border-white/30 bg-white/5 px-4 focus:ring-2 focus:ring-blue-500/20 text-white font-bold">
                    <option value="" className="bg-black">-- No Album (General) --</option>
                    {albums.map(album => <option key={album.id} value={album.id} className="bg-black">{album.title}</option>)}
                  </select>
                </div>
                <div className="flex items-end">
                  <Button 
                    ...
                    variant="outline" 
                    className="h-14 px-6 rounded-2xl border-white/30 flex items-center gap-2 hover:bg-black/10 text-white font-black uppercase text-[10px] tracking-widest transition-all"
                  >
            ...
            <div>
              ...
              <Input type="file" multiple accept="image/*" onChange={handleFileChange} className="w-full h-14 rounded-2xl pt-3.5 bg-white/5 border-white/30 text-white font-bold cursor-pointer file:bg-blue-600 file:text-white file:border-0 file:rounded-xl file:px-4 file:py-1 file:mr-4 file:font-black file:text-[10px] file:uppercase" />
            </div>

            {uploadingImg && (
              <div className="w-full bg-white/5 rounded-full h-4 overflow-hidden border border-white/30">
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-xs font-black uppercase text-white/50 mb-2 tracking-widest">Category</label>
              <select value={vidCategory} onChange={(e) => setVidCategory(e.target.value as Category)} className="w-full h-14 rounded-2xl border border-white/30 bg-white/5 px-4 focus:ring-2 focus:ring-blue-500/20 text-white font-bold">
            ...
            {vidUploadType === 'link' ? (
              <div key="link-upload">
                ...
                <Input 
                  ...
                  className="h-14 rounded-2xl placeholder:text-white/20 bg-white/5 border-white/30 text-white font-bold" 
                />
              </div>
            ) : (
              <div key="file-upload">
                ...
                <Input 
                  ...
                  className="h-14 rounded-2xl pt-3.5 bg-white/5 border-white/30 text-white font-bold cursor-pointer file:bg-blue-600 file:text-white file:border-0 file:rounded-xl file:px-4 file:py-1 file:mr-4 file:font-black file:text-[10px] file:uppercase" 
                />
                ...
                {uploadingVid && vidUploadType === 'file' && (
                  <div className="w-full bg-white/5 rounded-full h-4 overflow-hidden mt-6 border border-white/30">
      ...
      <div className="p-4 sm:p-8 rounded-3xl bg-transparent border border-white/30 shadow-2xl bento-card w-full">
         ...
         <div className="space-y-12">
            {CATEGORIES.map(cat => (
               <div key={cat} className="border-b border-white/20 pb-12 last:border-0 relative z-10">
        ...
        <Dialog open={showAlbumModal} onOpenChange={setShowAlbumModal}>
          <DialogContent className="sm:max-w-md border-white/30 bg-black/95 backdrop-blur-2xl text-white rounded-3xl">
            <DialogHeader>
            ...
            <div className="space-y-6 py-6">
              <div>
                ...
                <Input 
                  ...
                  className="h-14 rounded-2xl bg-white/5 border-white/30 text-white font-bold placeholder:text-white/20"
                />
              </div>
              <div>
                ...
                <textarea 
                  ...
                  className="w-full rounded-2xl border border-white/30 bg-white/5 p-4 min-h-[120px] text-sm text-white font-medium focus:ring-2 focus:ring-blue-500/20 placeholder:text-white/20"
                />
              </div>
            </div>
