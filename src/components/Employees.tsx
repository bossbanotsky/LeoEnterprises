      ...
      <div className="mb-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-[10px] font-black text-white uppercase tracking-widest mt-1">Total Workforce: {counts.all} Employees</p>
          ...
          <div className="flex bg-black p-1 rounded-xl border border-white/30 shrink-0">
            <button 
                className={`px-3 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all flex items-center gap-2 ${statusFilter === 'all' ? 'bg-black/10 text-white shadow-xl' : 'text-white hover:text-white'}`}
                onClick={() => setStatusFilter('all')}
            >
              All
              <span className={`px-1.5 py-0.5 rounded-md ${statusFilter === 'all' ? 'bg-black/20' : 'bg-white/5'}`}>{counts.all}</span>
            </button>
            <button 
                className={`px-3 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all flex items-center gap-2 ${statusFilter === 'active' ? 'bg-black/10 text-white shadow-xl' : 'text-white hover:text-white'}`}
            ...
            <button 
                className={`px-3 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all flex items-center gap-2 ${statusFilter === 'deactivated' ? 'bg-black/10 text-white shadow-xl' : 'text-white hover:text-white'}`}
        ...
        <div className="flex gap-2">
          <div className="relative group flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-white transition-colors group-focus-within:text-blue-500" />
            <Input 
              placeholder="Search employees..." 
              className="pl-10 bg-transparent border border-white/30 rounded-xl h-12 text-white placeholder:text-white focus-visible:ring-blue-500 transition-all font-bold"
              value={searchQuery}
          ...
          <select
            ...
            className="bg-black border border-white/30 rounded-xl px-3 h-12 text-[10px] font-black uppercase tracking-widest text-white focus:outline-none focus:border-blue-500 cursor-pointer"
          >
      ...
      <div className="flex-1 overflow-y-auto space-y-6 pb-20">
        {loading ? (
          ...
          sortedPositions.map(position => (
            <div key={position}>
              <h2 className="text-xs font-black text-white uppercase tracking-[0.2em] mb-4 pl-1 flex items-center gap-2">
              ...
              <div className="space-y-4">
                {groupedEmployees[position].map(emp => (
                    <Interactive 
                      ...
                      className="bg-white/5 backdrop-blur-xl p-4 flex flex-row items-center gap-4 border border-white/30 hover:border-blue-500 transition-all rounded-[24px] shadow-2xl relative overflow-hidden group"
                    >
                      ...
                      <div className="shrink-0 flex items-center gap-2">
                        <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-[0.15em] border transition-colors hidden xs:block ${
                          emp.status === 'active' || !emp.status 
                            ...
                            : 'bg-white/5 text-white/30 border-white/30'
                        }`}>
                          {(emp.status === 'inactive') ? 'Deactivated' : (emp.status || 'Active')}
                        </span>
                        <div className="w-8 h-8 rounded-full bg-white/5 border border-white/30 flex items-center justify-center group-hover:bg-blue-600 transition-colors">
        ...
        {!loading && filteredEmployees.length === 0 && (
          <EmptyState 
            ...
            description={searchQuery ? `No matches found for "${searchQuery}".` : "Your workforce directory is empty. Add your first employee to get started."}
      ...
      <Dialog open={isAddOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px] rounded-2xl">
          ...
          <div className="flex flex-col items-center justify-center py-4">
            <div 
              className="relative w-24 h-24 bg-black dark:bg-slate-800 rounded-2xl overflow-hidden border-2 border-dashed border-slate-300 dark:border-white/50 flex items-center justify-center cursor-pointer hover:border-blue-500 transition-colors group"
            ...
            <div className="space-y-6 mt-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-xl text-white dark:text-white">{selectedEmployee.fullName}</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-black dark:bg-slate-800 p-3 rounded-xl col-span-2">
                  <div className="text-xs text-white dark:text-white uppercase font-semibold mb-1">Employee ID</div>
                  <div className="font-bold text-white dark:text-white">{selectedEmployee.customId || 'No ID assigned'}</div>
                </div>
                <div className="bg-black dark:bg-slate-800 p-3 rounded-xl col-span-2">
                  <div className="text-xs text-white dark:text-white uppercase font-semibold mb-1">Email Address</div>
                  <div className="font-bold text-white dark:text-white truncate flex items-center gap-2">
                ...
                {selectedEmployee.loginPassword && (
                   <div className="bg-black dark:bg-slate-800 p-3 rounded-xl col-span-2">
                    <div className="text-xs text-white dark:text-white uppercase font-semibold mb-1 flex items-center gap-1">
                      <Key className="w-3 h-3" /> Login Password
                    </div>
                    <div className="font-mono text-xs font-bold text-white dark:text-white truncate">
                      {selectedEmployee.loginPassword}
                    </div>
                  </div>
                )}
                <div className="bg-black dark:bg-slate-800 p-3 rounded-xl">
                  <div className="text-xs text-white dark:text-white uppercase font-semibold mb-1">Daily Salary</div>
                  <div className="font-bold text-white dark:text-white">₱ {selectedEmployee.dailySalary.toFixed(2)}</div>
                </div>
                <div className="bg-black dark:bg-slate-800 p-3 rounded-xl">
                  <div className="text-xs text-white dark:text-white uppercase font-semibold mb-1">Hourly Rate</div>
                  <div className="font-bold text-white dark:text-white">₱ {selectedEmployee.hourlyRate.toFixed(2)}</div>
                </div>
              </div>
              ...
              {(selectedEmployee.birthday || selectedEmployee.sex || selectedEmployee.civilStatus || selectedEmployee.religion || selectedEmployee.sssNumber || selectedEmployee.philhealthNumber || selectedEmployee.pagibigNumber || selectedEmployee.tinNumber) && (
                <div className="pt-4 border-t border-white/20 dark:border-white/50">
                  <h4 className="text-sm font-bold text-white dark:text-white mb-3">Personal & Government Data</h4>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    {selectedEmployee.birthday && (
                      <div>
                        <span className="block text-[10px] uppercase font-bold text-white">Birthday</span>
                        <span className="font-medium text-white dark:text-white">{selectedEmployee.birthday}</span>
                      </div>
                    )}
                    {selectedEmployee.sex && (
                      <div>
                        <span className="block text-[10px] uppercase font-bold text-white">Sex</span>
                        <span className="font-medium text-white dark:text-white">{selectedEmployee.sex}</span>
                      </div>
                    )}
                    {selectedEmployee.civilStatus && (
                      <div>
                        <span className="block text-[10px] uppercase font-bold text-white">Civil Status</span>
                        <span className="font-medium text-white dark:text-white">{selectedEmployee.civilStatus}</span>
                      </div>
                    )}
                    {selectedEmployee.religion && (
                      <div>
                        <span className="block text-[10px] uppercase font-bold text-white">Religion</span>
                        <span className="font-medium text-white dark:text-white">{selectedEmployee.religion}</span>
                      </div>
                    )}
                    {selectedEmployee.sssNumber && (
                      <div>
                        <span className="block text-[10px] uppercase font-bold text-white">SSS Number</span>
                        <span className="font-medium text-white dark:text-white">{selectedEmployee.sssNumber}</span>
                      </div>
                    )}
                    {selectedEmployee.philhealthNumber && (
                      <div>
                        <span className="block text-[10px] uppercase font-bold text-white">PhilHealth No.</span>
                        <span className="font-medium text-white dark:text-white">{selectedEmployee.philhealthNumber}</span>
                      </div>
                    )}
                    {selectedEmployee.pagibigNumber && (
                      <div>
                        <span className="block text-[10px] uppercase font-bold text-white">Pag-IBIG / HDMF</span>
                        <span className="font-medium text-white dark:text-white">{selectedEmployee.pagibigNumber}</span>
                      </div>
                    )}
                    {selectedEmployee.tinNumber && (
                      <div>
                        <span className="block text-[10px] uppercase font-bold text-white">TIN Number</span>
                        <span className="font-medium text-white dark:text-white">{selectedEmployee.tinNumber}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {(selectedEmployee.emergencyContactName || selectedEmployee.emergencyContactRelation || selectedEmployee.emergencyContactPhone) && (
                <div className="pt-4 border-t border-white/20 dark:border-white/50">
                  <h4 className="text-sm font-bold text-white dark:text-white mb-3 text-red-600 dark:text-red-400">Emergency Contact</h4>
                  <div className="bg-red-50/30 dark:bg-red-900/10 p-3 rounded-xl border border-red-100/50 dark:border-red-900/20">
                    <div className="font-bold text-white dark:text-white">{selectedEmployee.emergencyContactName || 'No Name Provided'}</div>
                    <div className="flex items-center justify-between mt-1 text-sm">
                      <span className="text-white dark:text-white">{selectedEmployee.emergencyContactRelation || 'Relation Unknown'}</span>
              ...
              <div className="pt-4 border-t border-white/20 dark:border-white/50">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-medium text-white dark:text-white">Account Status</div>
