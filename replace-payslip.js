...
const fileToEdit = path.join(__dirname, 'src', 'components', 'Payroll.tsx');
...
const startToken = `className="p-8 payslip-mockup bg-white font-sans text-xs text-slate-900 border border-slate-200 rounded-xl select-none" \n                >`;
const endToken = `            </div>\n            </>`;
...
const before = content.substring(0, startIndex + startToken.length);
                                  ...
                                  <div key={dIdx} className="flex justify-between items-center text-[10px] pl-2 border-l border-purple-200">
                                    <div className="flex items-center gap-1.5">
                                      <Button 
                                        ...
                                        className={\`h-4 w-4 p-0 rounded-sm shrink-0 transition-all \${item.isPaid ? 'bg-purple-500 text-white' : 'bg-slate-100 border border-slate-200'}\`}
                                      >
...
console.log("Successfully replaced the inner content of payslipRef in Payroll.tsx!");
